...
RFC 8259 JSON parser, recursive serializer, JSONPath, JSON Schema validation, and NDJSON streaming for Alya
...
| `new_config(name, count)` | `name = "World", count = 1` | `JsonConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: JsonConfig` | `string` | Formats greeting using prefix and name from config. |
